...
public class UserQueryRepository {
    ...
    public UserEntity registerUser(RegisterUserRequestDto dto, String profilePhotoObjectKey, RoleEntity roleEntity){
        if (repository.existsByEmail(dto.email())) {
            throw new UserException("El correo electrónico ingresado ya está asociado a otro usuario. Por favor, utiliza una dirección diferente para continuar con el registro.");
        }
        return repository.save(new UserEntity(dto.name(), dto.email(), cryptoUtil.encode(dto.password()), profilePhotoObjectKey, roleEntity));
    }

    public UserEntity loginUser(String email, String password){
        Optional<UserEntity> optionalUserEntity = repository.findByEmail(email);
        if(optionalUserEntity.isEmpty()){
            throw new UserException("No encontramos ningún usuario con el correo ingresado. Por favor, verifica los datos y vuelve a intentarlo. Si aún no tienes una cuenta, puedes crear una nueva en el botón de Registro.");
        }
        UserEntity userEntity = optionalUserEntity.get();

        if(!cryptoUtil.matches(password, userEntity.getPassword())){
            throw new UserException("La autenticación no fue posible porque la contraseña proporcionada es incorrecta. Puedes volver a intentarlo o restablecerla desde la opción '¿Olvidaste tu contraseña?'.");
        }
        return userEntity;
    }
